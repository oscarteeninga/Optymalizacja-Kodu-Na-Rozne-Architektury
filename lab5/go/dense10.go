...
func chol(A matrix, n uint) int {
	...
	for j = 0; j < n; j++ {
		...
		for i = j; i < n; i += 8 {
			for k = 0; k < j; {
				if i < max(int(n-8), 0) && k < max(int(j-8), 0) {
					arr_i = A.m[i]
					tmp = arr_i[j]
					tmp = tmp - (arr_j[k] * arr_i[k])     /*OPT*/
					tmp = tmp - (arr_j[k+1] * arr_i[k+1]) /*OPT*/
					tmp = tmp - (arr_j[k+2] * arr_i[k+2]) /*OPT*/
					tmp = tmp - (arr_j[k+3] * arr_i[k+3]) /*OPT*/
					tmp = tmp - (arr_j[k+4] * arr_i[k+4]) /*OPT*/
					tmp = tmp - (arr_j[k+5] * arr_i[k+5]) /*OPT*/
					tmp = tmp - (arr_j[k+6] * arr_i[k+6]) /*OPT*/
					tmp = tmp - (arr_j[k+7] * arr_i[k+7]) /*OPT*/
					A.m[i][j] = tmp
