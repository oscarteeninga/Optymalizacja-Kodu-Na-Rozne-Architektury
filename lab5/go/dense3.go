		...
		for i = j; i < n; i++ {
			...
			for k = 0; k < j; {
				if k < max(int(j-blksize), 0) { /*OPT*/
					...
					tmp = tmp - (A[IDX(i, k+1, n)] * A[IDX(j, k+1, n)]) /*OPT*/
					tmp = tmp - (A[IDX(i, k+2, n)] * A[IDX(j, k+2, n)]) /*OPT*/
					tmp = tmp - (A[IDX(i, k+3, n)] * A[IDX(j, k+3, n)]) /*OPT*/
					tmp = tmp - (A[IDX(i, k+4, n)] * A[IDX(j, k+4, n)]) /*OPT*/
					tmp = tmp - (A[IDX(i, k+5, n)] * A[IDX(j, k+5, n)]) /*OPT*/
					tmp = tmp - (A[IDX(i, k+6, n)] * A[IDX(j, k+6, n)]) /*OPT*/
					tmp = tmp - (A[IDX(i, k+7, n)] * A[IDX(j, k+7, n)]) /*OPT*/
					k = k + blksize                                     /*OPT*/
				} else {
					tmp = tmp - (A[IDX(i, k, n)] * A[IDX(j, k, n)]) /*OPT*/
					k++
